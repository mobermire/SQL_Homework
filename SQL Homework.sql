...
SELECT title  AS 'Film Title', COUNT(i.film_id) AS 'Number of Copies'
FROM film f 
JOIN inventory i 
...
SELECT first_name AS 'First Name', last_name AS 'Last Name', SUM(p.amount) AS 'Total Paid'
FROM customer c 
JOIN payment p 
...
SELECT first_name AS 'FistName', last_name AS 'LastName', email
FROM customer
WHERE address_id IN(
	SELECT city.city_id
	FROM address a
	JOIN city
    ON a.address_id = city.city_id
...
FROM inventory i
JOIN(
	SELECT r.inventory_id, COUNT(r.inventory_id)
	FROM rental r
	GROUP BY r.inventory_id 
	ORDER BY COUNT(r.inventory_id) DESC) sub
ON i.inventory_id = sub.inventory_id
UNION ALL
SELECT title
FROM film;
...
#Since the staff table indicates each staff member is associated with only one store id, we can sum the total rung up by each staff member.
SELECT store_id, SUM(amount) as 'Total Rung Up'
FROM staff s
JOIN payment p
ON s.staff_id = p.staff_id
GROUP BY s.staff_id;
...
GROUP BY c.name ORDER BY SUM(p.amount) LIMIT 5;
